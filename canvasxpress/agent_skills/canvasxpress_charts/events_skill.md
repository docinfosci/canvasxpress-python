...
```python
# Single event - must be wrapped in CXEvents or a list
events = CXEvents(
    CXEvent(
        id="click",
...
> ```python
> # ALWAYS - single event wrapped in CXEvents
> events = CXEvents(CXEvent(id="click", script="var msg = o.y.vars[0]; t.showInfoSpan(e, msg);"))
>
> # ALWAYS - single event as a list
...
> ```
>
> **Remember:** The CanvasXpress Python API does NOT accept raw JavaScript objects. Only `CXEvents` objects or lists of `CXEvent` objects are accepted.
...
```python
# WRONG - CXEvent must be wrapped in CXEvents
events = CXEvent(id="click", script="...")  # Single CXEvent is NOT accepted

# CORRECT - single event wrapped in CXEvents
events = CXEvents(CXEvent(id="click", script="..."))

# CORRECT - single event as a list
...
# CORRECT - use CXEvents with CXEvent
events = CXEvents(CXEvent(id="click", script="var msg = o.y.vars[0]; t.showInfoSpan(e, msg);"))
```
...
# Single event - must use CXEvents or list
cx = CanvasXpress(
    data=data,
    config={"graphType": "Scatter2D", "title": "Interactive Chart"},
    events=CXEvents(CXEvent(id="click", script="var s = 'click on var ' + o.y.vars[0] + ' and smp ' + o.y.smps[0]; t.showInfoSpan(e, s);"))
)
graph(cx)

# Multiple events - use CXEvents
cx = CanvasXpress(
    data=data,
    config={"graphType": "Scatter2D", "title": "Interactive Chart"},
    events=CXEvents(
        CXEvent(id="click", script="var s = 'click on var ' + o.y.vars[0] + ' and smp ' + o.y.smps[0]; t.showInfoSpan(e, s);"),
        CXEvent(id="mousemove", script="t.showInfoSpan(e, '<pre>' + t.prettyJSON(o) + '</pre>');")
    )
)
graph(cx)

# Also valid: single event as a list
cx = CanvasXpress(
    data=data,
    config={"graphType": "Scatter2D", "title": "Interactive Chart"},
    events=[
...
cx = CanvasXpress(
    data=data,
    config={"graphType": "Bar"},
    events=CXEvents(CXEvent(id="click", script="var s = 'test'; t.showInfoSpan(e, s);"))
)
...
- [ ] **Events accepted** - Must use `CXEvents(...)` or list `[CXEvent(...), ...]` - a single `CXEvent` alone is NOT accepted
...
```python
# VALID - single event wrapped in CXEvents
events = CXEvents(CXEvent(id="click", script="var msg = o.y.vars[0]; t.showInfoSpan(e, msg);"))

# VALID - single event as a list
events = [CXEvent(id="click", script="var msg = o.y.vars[0]; t.showInfoSpan(e, msg);")]

# VALID - list of multiple CXEvent objects
events = [
    CXEvent(id="click", script="var msg = o.y.vars[0]; t.showInfoSpan(e, msg);"),
    CXEvent(id="mousemove", script="t.hideInfoSpan();")
]

# VALID - multiple events wrapped in CXEvents
events = CXEvents(
    CXEvent(id="click", script="var msg = o.y.vars[0]; t.showInfoSpan(e, msg);"),
    CXEvent(id="mousemove", script="t.hideInfoSpan();")
)

# INVALID - single CXEvent not wrapped
events = CXEvent(id="click", script="var msg = o.y.vars[0]; t.showInfoSpan(e, msg);")
...
> **Rule:** Every event must be `CXEvent(id="<string>", script="<string>")` where both `id` and `script` are strings. Single `CXEvent` objects must be wrapped in `CXEvents(...)` or a list `[...]`. No raw strings, dicts, or other formats are accepted.
