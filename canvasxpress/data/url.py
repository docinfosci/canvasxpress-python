...
class CXUrlData(CXData):
    ...
    def url(self) -> str:
        ...
        Returns:
            str: A string representation of the URL, similar to the data that will
    ...
    def url(self, url: str) -> None:
        """Tracks the specified URL.

        ValueError will be raised if the URL is malformed.

        Args:
            url: A string form of the URL, which must be in a standard format for
    ...
    def data(self, data: dict) -> None:
        ...
        Args:
            data: The URL as broken into a dict with the above attributes set to a
    ...
    def validate_url(cls, url: str, detail_errors: bool = True) -> bool:
        """Validates that the URL conforms to a recognized standard.

        URLs must begin with a valid scheme such as `file://` or `http://`.

        Args:
            url: The string form of the URL to be validated.
            detail_errors: True (default) if an exception should be raised with
                information detailing issues or errors, or `False` if the method
                should return `False` upon encountering issues.

        Returns:
            bool: True if the URL is valid, or `False` if issues are found in the
                URL.  If `detail_errors` is `True` then a ValueError will instead
                be raised with information pertaining to the rejection.

        Raises:
            ValueError: If the URL is invalid and `detail_errors` is `True`.
        """
        try:
    ...
    def __init__(self, data: str) -> None:
        """Initializes the CXUrlData object with a valid URL.

        URLs must begin with a valid scheme such as `file://` or `http://`.
        ValueError will be raised if the URL is malformed.

        Args:
            data: A string form of the URL, which must be in a standard format for
                files, http resources, ftp resources, etc.
        """
        super().__init__(data)
        self.url = data
