...
def test_set_valid_url():
    ...
    samples = ["https://www.canvasxpress.org/data/cX-basic-dat.txt"]
    for sample in samples:
        CXUrlData(sample)


def test_set_invalid_url():
    samples = ["snarf", "/test/that"]
    for sample in samples:
...
def test_canvasxpress_conversion():
    ...
    chart_options.set_param("title", "Tooth Growth").set_param(
        "smpLabelRotate", 90
    ).set_param("graphType", "Boxplot").set_param(
    ...
    chart = CanvasXpress(data=url_data, config=chart_options)
