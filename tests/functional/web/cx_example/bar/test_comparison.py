...
def test_js_vs_python_bar_example(app, tmp_path):
    ...
    js_url: str = url_for(
        "get_canvasxpress_js_chart",
        _external=True
    )
    python_url: str = url_for(
        "get_canvasxpress_python_chart",
        _external=True
    )
    ...
    with ChromeManagedBrowser(js_url) as js_browser:
        ...
        js_chart = js_browser.session.find_element_by_id(
            "canvasId"
        )
        js_chart.screenshot(
            str(js_chart_png)
        ...
        with ChromeManagedBrowser(python_url) as py_browser:
            ...
            py_chart = py_browser.session.find_element_by_id(
                "canvasId"
            )
            py_chart.screenshot(
                str(py_chart_png)
